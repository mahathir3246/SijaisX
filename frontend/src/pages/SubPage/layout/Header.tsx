...
import { get_substitute_info } from '../../../functions/api_calls';
...
const Navigation = ({title}:HeaderProps) => {
    ...
    useEffect(() => {
      const fetchUserName = async () => {
          ...
          if (subID) {
              try {
                  const subInfo = await get_substitute_info(subID);
                  if (subInfo && (subInfo as any).name) {
                      setUserName((subInfo as any).name);
                  }
              } catch (error) {
                  console.error('Error fetching substitute name:', error);
