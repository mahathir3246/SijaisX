...
export async function get_batch_of_accepted_assignments_for_substitute(substitute_ID: string) {
...
export async function get_completed_batches_for_teacher(teacher_ID: string, current_datetime: string) {
  ...
  try {
    const response = await fetch(url);
    if (!response.ok) {
      console.error(`Failed to fetch completed batches: ${response.statusText}`);
      return [];
    }
    const data = await response.json();
    return data.batches || [];
...
export async function add_substitute_to_batch(substituteID: string, batchID: string) {
    try {
        const response = await fetch(`${BASE_URL}/assignments/volunteer_batch`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ substitute_ID: substituteID, batch_ID: batchID }),
        });
